...
fun main() {
    ...
    // lista.any() retorna boolean se essa lista tem dados ou se está vazia.
    // lista.count() retorna o numero de elementos dentro dessa lista!
    ...
    val sumCalories = data.sumOf { it.calorias } // Qual a soma de calorias?
    println("A soma de calorias é: $sumCalories")


    val knowPanqueke = data.filter { it.nome == "Panqueca" }.any() // Sei como fazer panqueca? E sushi?
    ...
    val firstTwo = data.take(2) // Me dê as duas primeiras receitas
    for (x in firstTwo.withIndex()) {
        println("${x.index + 1} - ${x.value.nome}")
    }


    data.filter { it.calorias > 500 }.forEach { println(it.nome) } // Quais são as comidas com mais de 500 calorias?


    val maisCal = data.maxByOrNull { it.calorias } // Qual a receita mais calórica? E a menos calórica?
    ...
    data.map { it.nome } // Faça uma lista com o nome dos pratos


    val media = data.map { it.calorias }.average() // Qual a média de caloria de todas as receitas?
